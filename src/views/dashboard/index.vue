...
export default {
  ...
  methods: {
    ...
    addtopic() {
      this.quiz.questions.push({  standardAnswer: "",score:'' });
    ...
    fetchData() {
      ...
      if (query.replace == 1 && query.id) {
        getquizbyid(query.id).then(response => {
          this.quiz.name = response.value[0].name;
          this.quiz.markdown = response.value[0].markdown;
          const len = response.value[0].questions.length;
          console.log(len);
          for (let k = 0; k < len; k++) {
            this.quiz.questions.push({ questionType: "", standardAnswer: "" });
          }
          for (let k = 0; k < len; k++) {
            this.quiz.questions[k].questionType = response.value[0].questions[k].questionType;
            this.quiz.questions[k].standardAnswer = response.value[0].questions[k].standardAnswer;
          }
        });
      }
    }
