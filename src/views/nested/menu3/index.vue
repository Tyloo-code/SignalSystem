...
<script>
import { getcourses, putqcmap, deleteqcmap,getodataqcmap} from "@/api/nested";
import { coueseList, quizList } from "@/api/menu";
export default {
  name: "Menu3",
  data() {
    return {
  ...
  methods: {
    ...
    handleSearch() {
      ...
      if (this.form.quiz != "") {
        ...
        getodataqcmap(odatarout).then(response => {
    ...
    submitEdit() {
      for(let i =0;i<this.quizId.length;i++) {
        for(let j = 0;j <this.quizlist.length;j++) {
          ...
          this.course.quizzes.push(this.quizlist[i])
        }
        }
      }
    ...
    fetchData() {
      ...
      quizList().then(response => {
        this.quizlist = response.value;
      });
    }
  }
};
